...
func TestEncoder(t *testing.T) {
	...
	for _, tc := range []testcase{
		...
		},
		{
			...
			out:   `SEARCH test id,feedbacks.text,"test",false,["test"];`,
			shape: `SEARCH test id,feedbacks.text,"test",false,["test"];`,
		},
		{
			...
			out:   `SEARCH test WHERE {"text":{"$eq":"test"}};`,
			shape: `SEARCH test WHERE {"text":{"$eq":$1}};`,
			values: []dql.Expr{
				dql.NewStringExpr("test"),
			},
		},
		{
			...
			out:   `SEARCH operating_systems WHERE {"$and":[{"$not":[{"type":{"$eq":"unix"}}]},{"active":{"$eq":false}},{"kernel":{"$eq":"hybrid"}},{"release_date":{"$gte":"1990-01-01"}},{"release_date":{"$lte":"1991-01-01"}},{"$or":[{"author.name":{"$eq":"Rob Pike"}},{"author.name":{"$eq":"Ken Thompson"}},{"author.name":{"$eq":other.name}},{"release_date":{"$gte":"1990-01-01","$lte":"1991-01-01"}}]}]};`,
			shape: `SEARCH operating_systems WHERE {"$and":[{"$not":[{"type":{"$eq":$1}}]},{"active":{"$eq":$2}},{"kernel":{"$eq":$3}},{"release_date":{"$gte":$4}},{"release_date":{"$lte":$5}},{"$or":[{"author.name":{"$eq":$6}},{"author.name":{"$eq":$7}},{"author.name":{"$eq":$8}},{"release_date":{"$gte":$9,"$lte":$10}}]}]};`,
			values: []dql.Expr{
		...
		},
		{
			...
			out:   `SEARCH test WHERE {"labels":{"$in":["label-1","label-2"]}};`,
			shape: `SEARCH test WHERE {"labels":{"$in":$1}};`,
			values: []dql.Expr{
		...
		},
		{
			...
			out:   `SEARCH test WHERE {"$exists":"some.field"};`,
			shape: `SEARCH test WHERE {"$exists":"some.field"};`,
		},
		{
