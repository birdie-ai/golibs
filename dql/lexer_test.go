...
func TestLexer(t *testing.T) {
	...
	for _, tc := range []testcase{
		...
		},
		{
			...
			`,
			out: tokvals{
				token(identToken, "AS", 5, 1, 4),
				token(identToken, "s1", 8, 1, 7),
				token(identToken, "SEARCH", 16, 2, 5),
				...
				token(identToken, "ORDER", 238, 10, 5),
				token(identToken, "BY", 244, 10, 11),
				token(identToken, "posted_at", 247, 10, 14),
				token(identToken, "ASC", 257, 10, 24),
				token(identToken, "LIMIT", 266, 11, 5),
				token(numberToken, "10000", 272, 11, 11),
				token(identToken, "AGGS", 283, 12, 5),
				...
				token(identToken, "WITH", 394, 17, 5),
				token(identToken, "CURSOR", 399, 17, 10),
